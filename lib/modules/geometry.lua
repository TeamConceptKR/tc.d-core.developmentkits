...
--- @module geometry
--- Handles a large amount of geometry based maths functions. 
--- Most of these are pointless in regular scripting however they do have use. 
...
function geometry.is_point_on_line_segment(point, line_start, line_end)
    return geometry.distance_2d(point, line_start) + geometry.distance_2d(point, line_end) == geometry.distance_2d(line_start, line_end)
end
...
function geometry.calculate_slope(p1, p2)
    if p2.x - p1.x == 0 then return nil end
...
function geometry.angle_between_3_points(p1, p2, p3)
    local a = geometry.distance_3d(p2, p3)
    local b = geometry.distance_3d(p1, p3)
    local c = geometry.distance_3d(p1, p2)
...
function geometry.do_circles_intersect(c1_center, c1_radius, c2_center, c2_radius)
    return geometry.distance_2d(c1_center, c2_center) <= (c1_radius + c2_radius)
end
...
function geometry.is_point_in_circle(point, circle_center, circle_radius)
    return geometry.distance_2d(point, circle_center) <= circle_radius
end
...
function geometry.does_rect_intersect_line(rect, line_start, line_end)
    return geometry.do_lines_intersect({x = rect.x, y = rect.y}, {x = rect.x + rect.width, y = rect.y}, line_start, line_end) or geometry.do_lines_intersect({x = rect.x + rect.width, y = rect.y}, {x = rect.x + rect.width, y = rect.y + rect.height}, line_start, line_end) or geometry.do_lines_intersect({x = rect.x + rect.width, y = rect.y + rect.height}, {x = rect.x, y = rect.y + rect.height}, line_start, line_end) or geometry.do_lines_intersect({x = rect.x, y = rect.y + rect.height}, {x = rect.x, y = rect.y}, line_start, line_end)
end
...
function geometry.closest_point_on_line_segment(point, line_start, line_end)
    local l2 = geometry.distance_2d(line_start, line_end)^2
...
function geometry.is_point_in_sphere(point, sphere_center, sphere_radius)
    return geometry.distance_3d(point, sphere_center) <= sphere_radius
end
...
function geometry.do_spheres_intersect(s1_center, s1_radius, s2_center, s2_radius)
    return geometry.distance_3d(s1_center, s2_center) <= (s1_radius + s2_radius)
end
...
function geometry.rotation_to_direction(rotation)
...
function geometry.rotate_box(center, width, length, heading)
...
function geometry.calculate_rotation_matrix(heading, pitch, roll)
    local rad_heading = math.rad(heading or 0)
    local rad_pitch = math.rad(pitch or 0)
    local rad_roll = math.rad(roll or 0)
    return { { math.cos(rad_heading), -math.sin(rad_heading), 0 }, { math.sin(rad_heading), math.cos(rad_heading),  0 }, { 0, 0, 1 } }
end
...
function geometry.translate_point_to_local_space(point, box_origin, rot_matrix)
...
--- @return boolean: True if the point is inside the box, false otherwise.
function geometry.is_point_in_oriented_box(point, box)
    local rot_matrix = geometry.calculate_rotation_matrix(box.heading, box.pitch, box.roll)
    local translated_point = geometry.translate_point_to_local_space(point, box.coords, rot_matrix)
