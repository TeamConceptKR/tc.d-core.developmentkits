...
if not bdtk.is_server then
    ...
    local strings <const> = bdtk.get("lib.modules.strings")
    local maths <const> = bdtk.get("lib.modules.maths")
    local entities <const> = bdtk.get("lib.modules.entities")
    local requests <const> = bdtk.get("lib.modules.requests")

    --- Retrieves the license plate number of a vehicle.
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_plate(vehicle)
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_model(vehicle)
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_doors_broken(vehicle)
        local doors_broken = {}
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_windows_broken(vehicle)
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_tyre_burst(vehicle)
        local tyre_burst = {}
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_extras(vehicle)
        local extras = {}
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_custom_xenon_color(vehicle)
    ...
    function vehicles.get_vehicle_mod(vehicle, mod_type)
    ...
    --- Returns whether each neon light (left, right, front, back) is enabled.
    --- @param vehicle number: The handle of the vehicle.
    --- @return table: Table of booleans: { left, right, front, back }
    function vehicles.get_neon_enabled(vehicle)
        return {
    ...
    --- Gets extensive data for vehicle.
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_properties(vehicle)
        ...
        for mod_type = 0, 49 do
            mods[mod_type] = vehicles.get_vehicle_mod(vehicle, mod_type)
        end

        local properties = {
            model = GetEntityModel(vehicle),
            doors_broken = vehicles.get_doors_broken(vehicle),
            windows_broken = vehicles.get_windows_broken(vehicle),
            tyre_burst = vehicles.get_tyre_burst(vehicle),
            tyres_can_burst = GetVehicleTyresCanBurst(vehicle),
            plate = GetVehicleNumberPlateText(vehicle),
            plate_index = GetVehicleNumberPlateTextIndex(vehicle),
            body_health = maths.round(GetVehicleBodyHealth(vehicle), 1),
            engine_health = maths.round(GetVehicleEngineHealth(vehicle), 1),
            tank_health = maths.round(GetVehiclePetrolTankHealth(vehicle), 1),
            fuel_level = maths.round(GetVehicleFuelLevel(vehicle), 1),
            dirt_level = maths.round(GetVehicleDirtLevel(vehicle), 1),
        ...
        for extra_id = 0, 12 do
            if DoesExtraExist(vehicle, extra_id) then
            extras_state[tostring(extra_id)] = IsVehicleExtraTurnedOn(vehicle, extra_id) == 1
            end
        end
        ...
        local modifications = {
            ...
            pearlescent_color = select(1, GetVehicleExtraColours(vehicle)),
            wheel_color = select(2, GetVehicleExtraColours(vehicle)),
            wheels = GetVehicleWheelType(vehicle),
            window_tint = GetVehicleWindowTint(vehicle),
        ...
        local maintenance = {
            ...
            tyres = { burst_state = {}, burst_completely = {} },
        ...
        for i = 0, num_wheels - 1 do
            maintenance.tyres.burst_state[i] = IsVehicleTyreBurst(vehicle, i, false)
            maintenance.tyres.burst_completely[i] = IsVehicleTyreBurst(vehicle, i, true)
        end

        return properties, modifications, maintenance
    end

    --- Retrieves the class of a vehicle as a string.
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_class(vehicle)
        local classes = {
            [0] = 'compacts',
            [1] = 'sedans',
            [2] = 'suvs',
    ...
    --- @param vehicle number: The handle of the vehicle.
    function vehicles.get_vehicle_class_details(vehicle)
        if not vehicle then return nil end
        local class_id = GetVehicleClass(vehicle)
        local class_details = {
            class_id = class_id,
            class_name = vehicles.get_vehicle_class(vehicle), -- using the function we defined earlier
    ...
    function vehicles.get_vehicle_details(use_current_vehicle)
        ...
        if use_current_vehicle and IsPedInAnyVehicle(player, false) then
            ...
            vehicle, vehicle_data.distance = entities.get_closest_vehicle(player_coords, 5.0, false)
        ...
        local properties, modifications, maintenance = vehicles.get_vehicle_properties(vehicle)
        if not properties then
            return nil
        end

        vehicle_data.vehicle = vehicle
        vehicle_data.plate = properties.plate
        vehicle_data.model = properties.model
        vehicle_data.properties = properties
        vehicle_data.modifications = modifications
        vehicle_data.maintenance = maintenance
        vehicle_data.class = vehicles.get_vehicle_class(vehicle)

        local class_id = GetVehicleClass(vehicle)
        vehicle_data.class_details = {
            class_id = class_id,
        ...
        local rear_engine_models = { 'comet', 'infernusclassic', '811', 're7b', '9f', '9fcabrio', 'osiris', 'tyrus', 'adder', 'stinger', 'reaper', 'spacedocker', 'cometsafari', 'krieger', 'f19', 'injection', 'tropos' }
        vehicle_data.is_rear_engine = false
        for _, model_name in ipairs(rear_engine_models) do
            if string.lower(properties.model) == model_name then
    ...
    function vehicles.spawn_vehicle(vehicle_data)
