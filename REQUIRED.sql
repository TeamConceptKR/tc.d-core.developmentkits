CREATE TABLE IF NOT EXISTS `bdtk_users` (
...
CREATE TABLE IF NOT EXISTS `bdtk_bans` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `unique_id` VARCHAR(255) NOT NULL,
    `banned_by` VARCHAR(255) NOT NULL DEFAULT "bdtk",
    ...
    PRIMARY KEY (`id`),
    FOREIGN KEY (`unique_id`) REFERENCES `bdtk_users` (`unique_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE IF NOT EXISTS `bdtk_licences` (
...
CREATE TABLE IF NOT EXISTS `bdtk_xp` (
    `identifier` VARCHAR(255) NOT NULL,
